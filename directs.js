...
async function saveEditMessage() {
    ...
    try {
        // You MUST have a server endpoint like /api/directs/edit or similar
        // If you don't have one yet, I'll provide a mock fetch below
        await fetch(`/api/directs/edit`, {
            method: "POST",
            headers: {
        ...
        alert("Failed to edit message. Ensure server endpoint exists.");
    }
}
...
function switchDmTab(tab) {
    ...
    // Toggle Views
    const dmsSection = document.getElementById('dmListContainer'); // The container inside DMs view
    const groupsSection = document.getElementById('groupList');    // The container inside Groups view
...
function loadGroupList() {
    ...
    fetch("/api/groups/list", { credentials: "include" })
        ...
        .then(groups => {
            listContainer.innerHTML = "";
            if (groups.length === 0) {
                listContainer.innerHTML = "<div class='dm-empty-state'>No groups yet. Create one!</div>";
            } else {
                groups.forEach(g => {
                    const div = document.createElement("div");
                    ...
                    const onlineCount = g.members.filter(m => onlineUsers.has(m.username)).length;
